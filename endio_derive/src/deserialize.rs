...
use quote::quote;
use syn::{
    Data, DataEnum, DeriveInput, Fields, LitInt, WhereClause, parse_macro_input, parse_quote,
};

use crate::{get_field_padding, get_post_disc_padding, get_pre_disc_padding, get_trailing_padding};
...
pub fn derive(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    input
        .generics
        .params
        .push(parse_quote!(__ENDIO_ENDIANNESS: ::endio::Endianness));
    input
        .generics
        .params
        .push(parse_quote!(__ENDIO_READER: ::std::io::Read + ::endio::ERead<__ENDIO_ENDIANNESS>));
...
fn add_where_clauses_enum(where_clause: &mut WhereClause, data: &DataEnum, ty: &Ident) {
    where_clause
        .predicates
        .push(parse_quote!(#ty: ::endio::Deserialize<__ENDIO_ENDIANNESS, __ENDIO_READER>));
