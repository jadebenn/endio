...
use quote::quote;
use syn::{
    Data, DataEnum, DeriveInput, Fields, Generics, LitInt, WhereClause, parse_macro_input,
    parse_quote,
};
...
pub fn derive(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    input
        .generics
        .params
        .push(parse_quote!(__ENDIO_WRITER: ::std::io::Write + ::endio::EWrite<__ENDIO_ENDIANNESS>));
...
fn add_where_clauses_enum(where_clause: &mut WhereClause, data: &DataEnum, ty: &Ident) {
    where_clause
        .predicates
        .push(parse_quote!(#ty: ::endio::Serialize<__ENDIO_ENDIANNESS, __ENDIO_WRITER>));
